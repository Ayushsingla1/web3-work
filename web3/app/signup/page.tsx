  ...
  const GithubSignup = async(e : React.MouseEvent<HTMLButtonElement>)=>{
    ...
    <div className="flex min-h-screen">
      <div className="flex flex-col items-center justify-center w-2/5 p-12 bg-gradient-to-b from-gray-800 to-gray-700">
        ...
        <div className="bg-gray-600 p-8 rounded-lg w-full max-w-md h-[500px] flex flex-col justify-between shadow-lg">
          <form className="space-y-6 flex-grow mt-8">
            <div className="space-y-6">
              <div>
                <label htmlFor="name" className={`block text-[18px] font-medium text-[#F3F3F3] ${holtwoodOneSC.className}`}>NAME</label>
                <input type="text" id="name" name="name" value = {details.name} onChange={changeHandler} className="w-full px-3 py-2 mt-1 bg-gray-800 border border-gray-600 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 bg-[#D7E4F0]" />
              </div>
              <div>
                <label htmlFor="email" className={`block text-[18px] font-medium text-[#F3F3F3] ${holtwoodOneSC.className}`}>EMAIL</label>
                <input type="text" id="email" name="email" value={details.email} onChange={changeHandler} className="w-full px-3 py-2 mt-1 bg-gray-800 border border-gray-600 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 bg-[#D7E4F0]" />
              </div>
              <div>
          ...
          <p className={`mt-4 text-sm text-center ${poppins.className} text-black`}>
            Already have an account? <Link href="/login" className="text-blue-400 hover:underline font-semibold">Login</Link>
          </p>
        </div>
        <div className="flex justify-center mt-6 space-x-4">
          <button className="p-3 bg-white rounded-full shadow-md hover:shadow-lg transition-shadow duration-300 w-12 h-12 flex items-center justify-center" onClick={GoogleSignup}>
            <Image src="/images/google.svg" alt="Google" width={24} height={24} />
          </button>
          <button className="p-3 bg-white rounded-full shadow-md hover:shadow-lg transition-shadow duration-300 w-12 h-12 flex items-center justify-center" onClick={GithubSignup}>
